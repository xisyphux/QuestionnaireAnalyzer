...
def pre_process_data(questionnaire_data: pd.DataFrame, output_dir: str) -> pd.DataFrame:
    """
    ...
    selected_column.columns = selected_column.columns.str.replace('Q12_花卉、草坪、树木等植物观赏空间', 'Q12')
    selected_column.columns = selected_column.columns.str.replace('Q13_喝茶、阅读、冥想、休闲等休憩空间', 'Q13')
    selected_column.columns = selected_column.columns.str.replace('Q14_散步、慢跑、瑜伽、乒乓球、羽毛球等运动健康空间', 'Q14')
    selected_column.columns = selected_column.columns.str.replace('Q15_聊天、派对、沙龙等聚会空间', 'Q15')
    selected_column.columns = selected_column.columns.str.replace('Q16_新鲜无污染的瓜果蔬菜等农作物种植采摘空间', 'Q16')
    selected_column.columns = selected_column.columns.str.replace('Q17_冷餐会、果蔬轻食分享等餐饮空间', 'Q17')
    selected_column.columns = selected_column.columns.str.replace('Q18_果蔬集市、跳蚤市场等展示销售空间', 'Q18')
    selected_column.columns = selected_column.columns.str.replace('Q19_雨水收集、污水处理等水资源回收利用空间', 'Q19')
    selected_column.columns = selected_column.columns.str.replace('Q20_厨余垃圾堆肥、废旧物再利用等垃圾精细化分类空间', 'Q20')
    selected_column.columns = selected_column.columns.str.replace('Q21_光伏发电、太阳能照明等再生能源利用空间', 'Q21')
    selected_column.columns = selected_column.columns.str.replace('Q22_城市蜂房、生态鸟屋等动物友好空间', 'Q22')
    selected_column.columns = selected_column.columns.str.replace('Q23_识农教育、植物认知、昆虫观察等儿童自然教育空间', 'Q23')
    selected_column.columns = selected_column.columns.str.replace('Q24_体现社区文化的艺术装置、景观雕塑等', 'Q24')
    selected_column.columns = selected_column.columns.str.replace('Q25_画展、音乐会、节庆活动等艺文空间', 'Q25')
    ...
    cluster_column = selected_column.iloc[:, [0,1,2,4,5,8]]
    ...
    print('{:-^80}\n'.format(' Pre-process Data Done '))
    cluster_column.to_excel(os.path.join(output_dir, "cluster_column.xlsx"))
    selected_column.to_excel(os.path.join(output_dir, "whole_selected_column.xlsx"))

    return selected_column, cluster_column